    ...
    def select_next_agent(self, state: ResearchState) -> Optional[str]:
        ...
        if state.get("report_generated", False):
            ...
            if not state.get("needs_refinement", True):
                ...
                # ✅ CORRECTED ROUTE: Route directly to the Synthesis Agent for the rewrite.
...
class PubMedAgent:
    ...
    def execute(self, state: ResearchState) -> ResearchState:
        ...
        if pmids:
            ...
            state.setdefault('references', []).extend([f"📄 Journal Article: {r['metadata']['title']}" for r in standardized_data])
...
class MaterialsAgent:
    ...
    def execute(self, state: ResearchState) -> ResearchState:
        ...
        if raw_results:
            ...
            state.setdefault('raw_tool_data', []).extend(standardized_data)
            state.setdefault('references', []).extend([f"⚛️ Materials Project: {r['metadata']['material_id']} ({r['metadata']['formula']})" for r in standardized_data])
...
class WebAgent:
    ...
    def execute(self, state: ResearchState) -> ResearchState:
        ...
        if raw_results:
            ...
            state.setdefault('references', []).extend([f"🔗 Web Source: {r['metadata']['title']} ({r['metadata']['url']})" for r in standardized_results])
...
class RAGAgent:
    ...
    def execute(self, state: ResearchState) -> ResearchState:
        ...
        # 🟢 FIX: Set a more appropriate base threshold for RAG, especially for general/web content
        ...
        # 🟢 FINAL CRITICAL FIX (New logic): If the normal filtering yielded nothing, use the raw chunks.
        ...
        # 🟢 FIX: If context is still empty after RAG, put a message in the log, but allow Synthesis to try
    ...
    def _extract_references(self, state: ResearchState) -> str:
        ...
        for entry in raw_data:
            ...
            if source_type == 'web_search' and metadata.get('url'):
                ...
                ref_snippet = f"🔗 Web Source: {metadata.get('title')}"

            elif source_type == 'arxiv' and metadata.get('pdf_url'):
                url = metadata['pdf_url']
                ref_snippet = f"🔗 Arxiv: {metadata.get('title')}"
                ...
                if url:
                    ...
                    ref_snippet = f"📄 Journal Article: {metadata.get('title')}"
        ...
        lookup_prefixes = ('📄 Journal Article:', '🔗 Arxiv:')
